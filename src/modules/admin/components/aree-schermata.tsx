...
/**
 * La schermata Aree del pannello admin. Era tutto dentro admin-client.tsx
 * finche' il pannello aveva una schermata sola; da quando ne ha due, il
 * guscio (rail, guardia, splash) sta in admin-client e ogni voce del menu
 * e' un file suo.
 *
...
export function AreeSchermata({
  righe,
  setRighe,
}: {
  righe: Riga[];
  setRighe: (f: (prev: Riga[]) => Riga[]) => void;
}) {
  const t = useT();
      ...
      {rigaAperta && (
        <section className="jm-adm-panel">
          ...
          <div className="jm-adm-actions">
            <button type="button" className="jm-adm-btn ghost" onClick={() => setAperta(null)}>
              {t("Chiudi")}
            </button>
          </div>
        </section>
