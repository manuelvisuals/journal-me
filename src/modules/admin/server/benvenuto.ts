 ...
 *
 * CHI ENTRA: `requireAdmin` dello scheletro (src/lib/server/entitlement.ts),
 * la stessa guardia di tutte le voci del pannello. L'email
 * dell'amministratore non puo' stare scritta in due posti.
 *
...
export async function PUT(req: NextRequest) {
  ...
  // Una pagina del sito stesso (/support) vale quanto un indirizzo esterno:
  // e anzi il caso normale. Cio che non si accetta e una stringa che non e
  // un indirizzo: aprirebbe una scheda vuota in faccia a chi ha appena
  // letto "scrivimi".
  if (url !== "" && !/^(https?:\/\/|mailto:|\/[^/])/i.test(url)) {
    return NextResponse.json(
      { error: "L'indirizzo deve cominciare con / oppure https:// oppure mailto:" },
      { status: 400 },
    );
  }
