...
export async function requirePremium(
  ...
  // COMPRARE SENZA ACCOUNT (Apple 5.1.1(v), bocciatura del 14 settembre
  // 2026; decisione di Manuel del 15). Il premium comprato SENZA email
  // (migration 025) vive sul braccialetto del telefono: se non c'e un
  // gettone ma c'e il braccialetto e ha un premium valido, e premium.
  // Questa seconda porta era stata chiusa il 10 settembre ("premium vuole
  // un account") e Apple l'ha fatta riaprire: una registrazione non si puo
  // pretendere prima di un acquisto in-app. Import dinamico: ospite.ts
  // importa questo file, e un ciclo statico fra i due confonderebbe il
  // bundler.
  const header = req.headers.get("authorization") ?? "";
